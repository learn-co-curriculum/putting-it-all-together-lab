...
export default (state=initialState, action) => {
  switch (action.type) {
    ...
    case 'SET_AI_CARDS':
      ...
      var card1 = newDeck.splice(randomIndex, 1)[0]
      var card2 = newDeck.splice(randomIndex, 1)[0]
      const newAiCards = [...state.aiCards, card1, card2]
      return Object.assign({}, state, {deck: newDeck, aiCards: newAiCards})
    case 'SET_USER_CARDS':
      var newDeck = [...state.deck]
      var randomIndex = Math.floor(Math.random() * state.deck.length)
      var card1 = newDeck.splice(randomIndex, 1)[0]
      var card2 = newDeck.splice(randomIndex, 1)[0]
      const newUserCards = [...state.userCards, card1, card2]
      return Object.assign({}, state, {deck: newDeck, userCards: newUserCards})
    default:
      return state
  }
}
